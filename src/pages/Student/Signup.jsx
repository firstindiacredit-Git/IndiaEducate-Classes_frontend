...
const Signup = () => {
  ...
  const handleSignup = async (values) => {
    setLoading(true);
    try {
      const res = await axios.post(`${import.meta.env.VITE_BASE_URL}/api/student/signup`, values);
  ...
  const handleVerifyOTP = async (values) => {
    ...
    try {
      ...
      // Check if this was a new signup (account created) or existing user login
      if (res.data.message === 'OTP verified, account created successfully') {
        message.success('Account created successfully! Please complete your profile.');
        setShowOTP(false);
        form.resetFields();
        otpForm.resetFields();
  ...
  return (
    <div style={{ maxWidth: 400, margin: '40px auto', padding: 24, background: '#fff', borderRadius: 8, boxShadow: '0 2px 8px #f0f1f2' }}>
      <Title level={3} style={{ textAlign: 'center' }}> Student Signup</Title>
      {!showOTP ? (
        <>
          ...
          <div style={{ textAlign: 'center', marginTop: 16 }}>
            <Typography.Text>Already have an account? <Link to="/">Login</Link></Typography.Text>
          </div>
        </>
      ) : (
