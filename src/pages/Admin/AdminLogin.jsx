...
const AdminLogin = () => {
  ...
  const handleLogin = async (values) => {
    setLoading(true);
    try {
      const res = await axios.post(`${import.meta.env.VITE_BASE_URL}/api/admin/login`, values);
      ...
      {!showOTP ? (
        <>
          ...
          <div style={{ textAlign: 'center', marginTop: 16 }}>
            <Typography.Text>Don't have an account? <Link to="/admin-signup">Signup</Link></Typography.Text>
          </div>
            ...
            {forgotStep === 1 && (
              <Form
                ...
                onFinish={async (values) => {
                  ...
                  try {
                    await axios.post(`${import.meta.env.VITE_BASE_URL}/api/admin/forgot-password`, { email: values.email });
          ...
          >
            <Form
              ...
              onFinish={async (values) => {
                setForgotLoading(true);
                try {
                  await axios.post(`${import.meta.env.VITE_BASE_URL}/api/admin/reset-password`, {
                    email: forgotEmail,
                    otp: forgotOtp,
